...
% Input design name
des_name = ['02062026PropPhi06HYBRID']; 
...
s = importdata([dr.geom des_name '-mission.json'],'r');
 ...
 %ScaleFactor = mymission.ScaleFactor; %scales velocites so that W1tip =100 m/s

% Flight operating points for mesh_far
Vf = [1 20 40]; alt = [0 0 3000]; leg = {'Static' 'Take-Off'  'Cruise'}; %alt = zeros(size(Vf));


% RPMs for all points on the mission
rpm_all = [6952 8191 8545];
...
%strut = readmatrix([dr.geom 'NACA0012-rescaled.csv']);
strut = dlmread([dr.geom 'NACA0030.csv'],',',1,0);
x = strut(:,1); y = strut(:,2); z = strut(:,3);
[polstrut_RT, polstrut_X, polstrut_R] = cart2pol(x,y,z);
polstrut = [polstrut_X, polstrut_RT, polstrut_R];
S{1}.xrrt = reshape(strut(:,[1 3 2]),81,[],3);
%S{1}.xrrt = reshape(strut(:,[1 3 2]),122,[],3);
%nn = 1; S(2) = [];
...
%figure(); hold on; grid on; box on; axis equal; cols = lines(length(B));
%for n = 1:length(S)
%    plot3(S{n}.xrrt(:,:,1),S{n}.xrrt(:,:,2),S{n}.xrrt(:,:,3),'-','color',cols(n,:))
%end
...
%dx_duct = 2 * xchord_max; M.dx_row = 0.2 * xchord_max;

%xchord_max = max(max(B{1}.xrrt(:,1,1)) - min(B{1}.xrrt(:,1,1)));
%xchord_max = max((max(B{1}.xrrt(:,:,1),[],1) - min(B{1}.xrrt(:,:,1),[],1)));
%dx_duct = 1.5* xchord_max; 
%M.dx_row = 0.1 * xchord_max; dx_row = M.dx_row;
%M.dx_row = 0.2*dx_duct; dx_row = M.dx_row;

%M.dx_row = 0.2 * dx_duct;
%dx_row = 0.2 * dx_duct;
...
S{1}.x_in = min(min(S{1}.xrrt(:,:,1))) - dx_duct; 
S{end}.x_out = max(max(S{end}.xrrt(:,:,1))) + dx_duct;
y = linspace(S{1}.x_in,S{end}.x_out,101)'; 
sxr_hub = [y ones(size(y)) * 0.1]; sxr_cas = [y ones(size(y)) * 0.76];
if S{1}.xrrt(1,end,2) < 0.76
    sxr_hub = [y ones(size(y)) * (S{1}.xrrt(1,1,2) + 0.1e-3)];
    sxr_cas = [y ones(size(y)) * (S{1}.xrrt(1,end,2) - 0.1e-3)];
end

for n = 1:length(S); S{n}.xr_hub = sxr_hub; S{n}.xr_cas = sxr_cas; end;
...
%Save the blade as an ibl
bl_write_ibl([dr.geom des_name '.ibl'],B{1}, 0, 0)
bl_write_ibl([dr.geom  'BIGSTRUT.ibl'],S{1}, 0, 0, 3)
...
%phi for each condition
%phi = [0.48 0.6 0.69];
%Vx_mean = [36.6, 36.99, 52.58];



I.pout = mymission.Atm_Pressure; I.k = 10; I.rpm = [mymission.RPM 0]; %I.rpm = [mymission.RPM*ScaleFactor 0]; %I.rpm = [mymission.RPM 0];
I.k = 10;
...
if isempty(dir([dr.ts des_name '_avg.*'])) == 1
    job = bl_create_machine(B,dr,[job.name '_'],job,I,M,F,D,1,1,0,0);
end
%job = bl_create_machine(B,dr,[job.name '_'],job,I,M,F,D,1,1,0,0);
...
%write ibl or nosecone and tail cone
bl_write_ibl([dr.geom des_name 'nosecone.ibl'],c, 0, 0)
...
%     % Apply boundary conditions for different flight speeds
     g = ts_apply_inlet(g,[Po Ta 0 0],0); %Ask James if this should be T0
     for bid = ts_get_bids(g)
         for pid = ts_get_pids(g,bid)
             if g{bid+1}.patch{pid+1}.attribute.kind == 1
                 ...
                 %g{bid+1}.patch{pid+1}.ipout = 0; % < --------------------------------------------------- Check with James
     ...
     % Improve stability
     g{1}.av.cfl = 0.3;
     g{1}.av.turbvis_lim = 10000;
     g{1}.av.poisson_nsmooth = 20;
     g{1}.av.nomatch_int = 1;
     for bid = ts_get_bids(g)
     ...
     % Set hubs to stationary to remove possibility of problem with slipwalls
     g{end}.bv.rpmj1 = 0; g{end-1}.bv.rpmj1 = 0;

     % Set different RPM values for each operating point
     varnames = {'rpm' , 'rpmi1' , 'rpmi2' , 'rpmj1' , 'rpmj2' , 'rpmk1' , 'rpmk2'};
     for v = 1:length(varnames)
         for bid = ts_get_bids(g)
             if g{bid+1}.bv.(varnames{v}) ~= 0
                 g{bid+1}.bv.(varnames{v}) = rpm_all(n);
             end
         end
     end
...
% %         des_name = [des_name '_XL'];
     % g{1}.av.nstep = 14000; 
     % des_name = [des_name '_' num2str(g{1}.av.nstep)];
     % des_name = [des_name '_wall'];
     job.walltime = 1;
%     % Submit the job
     %job.name = des_name;
...
%% Restart cases now that the outer flowfield is established
...
for n = 1:length(Vf)
    ...
    g = ts_read_hdf5([dr.ts des_name '_O' num2str(n) '.hdf5']);
    ...
    Po = Pa + 0.5 * ro * Vf(n)^2;
    To = Ta + 0.5 / cp * Vf(n)^2;
    % Turn upper slipwalls into farfield boundary condition
    for bid = ts_get_bids(g)
        for pid = ts_get_pids(g,bid)
            if g{bid+1}.patch{pid+1}.attribute.kind == 7 && ...
                g{bid+1}.patch{pid+1}.attribute.jst ~= 0
    ...
    % Turn inlet into farfield boundary condition
    for bid = ts_get_bids(g)
        for pid = ts_get_pids(g,bid)
            if g{bid+1}.patch{pid+1}.attribute.kind == 0
                g{bid+1}.patch{pid+1}.attribute.kind = 13;
    ...
    % Run poisson loop again now there are no slipwalls
    g{1}.av.poisson_nstep = I.poisson_nstep;
    
    % Increase number of steps for averaging
    g{1}.av.nstep = 2.0e5; g{1}.av.nstep_save_start = 0.75e5; 
    
    % Submit the job
    %job.name = [des_name 'restart'];
    job.name = [des_name 'restart_O' num2str(n)];
    Jobs{end+1,1} = ts_sbatch(job,g);
...
for n = 1:length(Vf)
    ...
    % Read previous solution
    g = ts_read_hdf5([dr.ts des_name 'restart_O' num2str(n) '.hdf5']);

    % Increase number of steps for averaging
    g{1}.av.nstep = 2.0e5; g{1}.av.nstep_save_start = 1e5;
    g{1}.av.nchange = 0;

    
    % Submit the job
    %job.name = [des_name 'restart'];
    job.name = [des_name 'restart2' '_O' num2str(n)];
    Jobs{end+1,1} = ts_sbatch(job,g);
...
% Read previous solution
%g = ts_read_hdf5([dr.ts des_name 'restart' '.hdf5']);

% Increase number of steps for averaging
%g{1}.av.nstep = 2.0e5; g{1}.av.nstep_save_start = 1e5;
%g{1}.av.nchange = 0;

%job.name = [des_name 'restart2'];
%Jobs{end+1,1} = ts_sbatch(job,g);
