...
% V1 design from Sam
% des_name = 'IPM5_V1C'; phi = 0.7; q = 0.28; Dtip = 0.12; sig = 0.9; AR = 2; rpm = 7353;
des_name = 'IPM5_V1I'; phi = 0.6969; q = 0.353125; Dtip = 0.16; sig = 1; AR = 2.5; rpm = 11000;
des_name = 'IPM5_V1I'; phi = 0.7; q = 0.353125; Dtip = 0.16; sig = 1; AR = 2.5; rpm = 18000;
...
%     end
    ...
    B{1}.drdx_cas_le = interp1(c.xr_int(:,1),drdx_cowl,B{1}.xrrt(1,42,1));
    ...
    if B{2}.xrrt(1,42,1) > c.xr_exit(1,1)
        r_2 = interp1(c.xr_exit(:,1),c.xr_exit(:,2),B{2}.xrrt(1,42,1));
        B{2}.drdx_cas_le = interp1(c.xr_exit(:,1),drdx_exit,B{2}.xrrt(1,42,1));
    else
        r_2 = r_cas; drdx_cas_le = 0;
    end
    B{2}.A_le = pi * (r_2^2 - r_1^2); B{2}.r_le = 0.5 * (r_1 + r_2);
    r_1 = interp1(c.xr_tail(:,1),c.xr_tail(:,2),B{2}.xrrt(301,2,1));
    r_2 = interp1(c.xr_exit(:,1),c.xr_exit(:,2),B{2}.xrrt(301,42,1));
    ...
    B{2}.drdx_hub_te = interp1(c.xr_tail(:,1),drdx_tail,B{2}.xrrt(301,2,1));
    B{2}.drdx_cas_te = interp1(c.xr_exit(:,1),drdx_exit,B{2}.xrrt(301,42,1));
...
%     end
    ...
    filler = [999.0; 999.0];
    domain = [   -0.2, 0.25,   0.00000,  0.2 ];

    fileID = fopen('walls.IPM','w');
    fprintf(fileID,'%6s \n','The IPM 5');
    fprintf(fileID,'%10.3f\t%10.3f%10.3f\t%10.3f\n',domain);
    fprintf(fileID, '%10.8f\t%10.8f\n',flipud(c.xr_tail(1:10:end,:))');
    fprintf(fileID,'%10.8f\t%10.8f\n',flipud(b.xr_hub(1:10:end,:))');
    fprintf(fileID, '%10.8f\t%10.8f\n',flipud(c.xr_nose(1:10:end,:))');
    fprintf(fileID, '%10.8f\t%10.8f\n', filler);
    var1 = c.xr_nose(1:10:end,:);
    var2= b.xr_hub(1:5:end,:);
    var3 = c.xr_tail(1:10:end,:);
    fprintf(fileID,'%10.8f\t%10.8f\n',[c.xr_cowl(end,1); c.xr_cowl(end,2)]');
    fprintf(fileID, '%10.8f\t%10.8f\n',flipud(c.xr_cowl(1:10:end,:))');
    fprintf(fileID,'%10.8f\t%10.8f\n',b.xr_cas(1:10:end,:)');
    fprintf(fileID,'%10.8f\t%10.8f\n',c.xr_exit(1:10:end,:)');
    fprintf(fileID,'%10.8f\t%10.8f\n',[c.xr_cowl(end,1); c.xr_cowl(end,2)]');
    ...
    figure();
    hold on; 
    %plot(c.xr_exit(:,1),c.xr_exit(:,2),'-','color',cols(2,:))
    plot(c.xr_cowl(end,1),c.xr_cowl(end,2),'o','color',cols(2,:))
    plot(c.xr_cowl(:,1),c.xr_cowl(:,2),'-','color',cols(2,:))
    plot(b.xr_cas(:,1),b.xr_cas(:,2),'-','color',cols(2,:))
    plot(c.xr_exit(:,1),c.xr_exit(:,2),'-','color',cols(2,:))
    plot(c.xr_cowl(end,1),c.xr_cowl(end,2),'o','color',cols(2,:))

    plot(var1(:,1),var1(:,2),'-','color',cols(2,:))
    plot(var2(:,1),var2(:,2),'-','color',cols(2,:))
    plot(var3(:,1),var3(:,2),'-','color',cols(2,:))
...
if run_des == 1
    ...
    % Load design
%     load([dr.geom des_name '.mat']); job.name = des_name;
    load([dr.geom des_name 'E_splines.mat']); job.name = [des_name 'E'];
    
    % Domain size
    B{1}.x_in = 0.03; B{2}.x_out = 0.1;
...
%     I.guess = [dr.ts 'IPM5_guess.dat'];
    I%.guess = [dr.ts 'IPM5_V1CE_avg.hdf5'];

    % Mesh and run the design point in TURBOSTREAM - need to prevent stator and IGV flipping
...
%     end
    ...
    coords = [2 7 ; 7 19];
    ts_run_chic(job,coords,1,[],0,0.005)
...
if mesh_lift == 1
    ...
    g = ts_read_hdf5([dr.ts des_name 'E.hdf5']);
...
if mesh_cowl == 1
    ...
    M.n_te = 25; M.dt = 60; M.n_k = 25; M.r_min = 0.002; M.d_fac = 1.5; M.d_lip = 0.8;
...
if plot_pressure == 1
    ...
    % Block ids
    bid_noz = 16; bid_bl = 15; bid_in = 14; bid_sta = 7;
...
if write_pv == 1
    ...
    for n = 1:length(leg)
        ...
        ts_export_paraview(g,[dr.pv des_name 'E_O' num2str(n) '.hdf5'],'HighSpeed',[],0);        
